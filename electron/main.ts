...
async function createSession(shapeId: string): Promise<void> {
  if (sessions.has(shapeId)) return;

  const authStorage = AuthStorage.create();
  const modelRegistry = ModelRegistry.create(authStorage);

  const model = getModel("anthropic", "claude-opus-4-6");
  if (!model) throw new Error("Model claude-opus-4-6 not found");
...
function destroySession(shapeId: string): void {
...
ipcMain.handle("agent:create-session", async (_event, shapeId: string) => {
  try {
...
ipcMain.handle("agent:prompt", async (_event, shapeId: string, text: string) => {
  const session = sessions.get(shapeId);
  if (!session) {
...
ipcMain.handle("agent:destroy-session", async (_event, shapeId: string) => {
  destroySession(shapeId);
  return { success: true };
});

// Canvas query response from renderer
ipcMain.on("canvas:query-response", (_event, id: string, result: any) => {
